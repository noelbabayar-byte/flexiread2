...
class PDFProcessor:
    ...
    def process_page(self, page_num: int) -> Dict[str, Any]:
        ...
        if len(text) >= self.MIN_TEXT_LENGTH:
            ...
            logger.debug(
                f"Page {page_num + 1}: Triggering OCR (only {len(text)} chars found)"
            )
        ...
        try:
            for page_num in range(self.total_pages):
                ...
                if (page_num + 1) % 50 == 0:
                    gc.collect()
                    logger.debug(
                        f"Garbage collection triggered after page {page_num + 1}"
                    )
...
def process_pdf_file(
    ...
    try:
        with PDFProcessor(pdf_path) as processor:
            ...
            logger.info(
                f"PDF processing successful: {direct_pages} direct, {ocr_pages} OCR"
            )
            return result, None
